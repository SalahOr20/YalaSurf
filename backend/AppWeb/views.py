...
from .models import SurfClub, CustomUser, Monitor, Equipment, SurfSpot, LessonSchedule, SurfLesson, Order, Surfer, \
    SurfSession, EquipmentSelection, OrderItem, Forum, Message
from .serializer import CustomUserSerializer, SurferSerializer, SurfClubSerializer, MonitorSerializer, \
    ...
    SurfSessionSerializer, MessageSerializer, ForumSerializer
    ...
    if request.method == 'POST':
        ...
        if user_serializer.is_valid():
            ...
            if role == 'surfer':
                ...
                surfer_data = {
                    'user_id': user.id,  # Associe l'ID de l'utilisateur
...
def surfclub_LessonSchedules(request):
    try:
        ...
        lessonSchedules_serializer = SurfSpotSerializer(lessonSchedules, many=True)
        return Response({
...
def surfclub_SurfSessions(request):
    try:
        ...
        surf_sessions = SurfSession.objects.filter(surf_club=surf_club)
        surf_sessions_serializer = SurfSessionSerializer(surf_sessions, many=True)

        # Récupérer tous les horaires de cours associés au club de surf
        lesson_schedules = LessonSchedule.objects.filter(surf_club=surf_club)
        lesson_schedules_serializer = LessonScheduleSerializer(lesson_schedules, many=True)

        # Récupérer tous les moniteurs associés au club de surf
        monitors = Monitor.objects.filter(surf_club=surf_club)
        monitors_serializer = MonitorSerializer(monitors, many=True)

        return Response({
            'surf_club': surf_club.name,
            'surf_sessions': surf_sessions_serializer.data,
            'lesson_schedules': lesson_schedules_serializer.data,
            'monitors': monitors_serializer.data
        }, status=status.HTTP_200_OK)
...
def surfclub_SurfSession(request, pk):
    try:
        # Récupérer la session de surf basée sur l'ID
        surf_session = SurfSession.objects.get(id=pk)

        # Sérialisation des données de la session de surf
        surf_session_serializer = SurfSessionSerializer(surf_session)

        # Récupérer et sérialiser les informations associées
        lesson_schedule_serializer = LessonScheduleSerializer(surf_session.lesson_schedule)
        monitor_serializer = MonitorSerializer(surf_session.monitor)

        return Response({
            'surf_session': surf_session_serializer.data,
            'lesson_schedule': lesson_schedule_serializer.data,
            'monitor': monitor_serializer.data
        }, status=status.HTTP_200_OK)
...
def surfclub_SurfLessons(request):
    try:
        surf_club=SurfClub.objects.get(user=request.user)
        surflessons = SurfLesson.objects.filter(surf_club=surf_club)
        surflessons_serializer = SurfLessonSerializer(surflessons, many=True)
        return Response({
        'SurfLessons': surflessons_serializer.data
    }, status=status.HTTP_200_OK)
    except SurfLesson.DoesNotExist:
        return Response({"error": "SurfLessons not found."}, status=status.HTTP_404_NOT_FOUND)
...
def surfclub_SurfLesson(request,pk):
    try:
        surflesson = SurfLesson.objects.get(pk=pk)
        SurfLesson_serializer = LessonScheduleSerializer(surflesson)
        return Response({
            'SurfLesson': SurfLesson_serializer.data
        }, status=status.HTTP_200_OK)
    except SurfLesson.DoesNotExist:
...
def surfclub_orders(request):
    try:
        # Récupérer le surf club de l'utilisateur actuel
        surf_club = SurfClub.objects.get(user=request.user)

        # Récupérer tous les équipements appartenant à ce surf club
        club_equipments = Equipment.objects.filter(surf_club=surf_club)

        # Récupérer toutes les commandes associées à ces équipements
        orders = Order.objects.filter(items__equipment__in=club_equipments).distinct()
...
def create_surf_session(request):
    try:
        # Retrieve the SurfClub for the authenticated user
        surf_club = SurfClub.objects.get(user=request.user)

        # Copy the data from the request and set the surf_club ID
        surf_session_data = request.data.copy()
        surf_session_data['surf_club'] = surf_club.id

        # Get a list of inactive monitors in the surf club
        ...
        # Randomly select an inactive monitor
        monitor = random.choice(inactive_monitors)
        surf_session_data['monitor'] = monitor.id

        # Validate and save the SurfSession instance
        surf_session_serializer = SurfSessionSerializer(data=surf_session_data)
        if surf_session_serializer.is_valid():
            # Save the SurfSession
            surf_session = surf_session_serializer.save()

            # Update the selected monitor to be active
            monitor.active = True
            monitor.save()

            return Response(surf_session_serializer.data, status=status.HTTP_201_CREATED)

        return Response(surf_session_serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
def get_surfclub_lesson(request, pk):
    try:
        ...
        surfsessions_serializer = SurfSessionSerializer(surfsessions, many=True)
...
def create_order(request):
    try:
        with transaction.atomic():
            ...
            order = Order.objects.create(surfer=surfer)
            ...
            for equipment_id in equipment_ids:
                try:
                    ...
                    return Response({"error": f"Equipment with ID {equipment_id} not found."},
                                    status=status.HTTP_404_NOT_FOUND)

                # Vérifier si l'équipement est déjà vendu
                if equipment.is_sell:
                    return Response({"error": f"Equipment {equipment.name} is already sold."},
                                    status=status.HTTP_400_BAD_REQUEST)
